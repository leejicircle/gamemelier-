...
> 현황(2026-07 갱신): 이 문서는 **초기 계획서(2026-06-12)**다. 이후 v1~v2-3(취향 칩·이유 배지·선반 B) + 온보딩 게임 픽커 + 세부 태그 노출(상세·취향칩) + 추천 스코어링 재균형(러프한 장르 완화·세밀한 태그 우대) + 픽커 장르 다양성까지 **배포 완료**. v3(임베딩)·CF는 대기.
> ⚠️ **구현 현황·회고·의사결정은 Notion "추천 개인화 구현 노트"가 living doc.** 이 계획서는 역사적 설계 기록으로 유지.
