...
// 클라이언트(브라우저)에서만 호출된다 — useTopSellerCards 훅.
// 서버 측(page.tsx)은 getTopSellerIds()를 직접 쓰므로 self-fetch 가 없고,
// 브라우저에서는 상대경로로 충분하다(NEXT_BASE_URL 불필요).
export async function fetchTopSellerIds(limit = 30, offset = 0) {
  const res = await fetch(`/api/topsellers?limit=${limit}&offset=${offset}`, {
    cache: 'no-store',
  });
